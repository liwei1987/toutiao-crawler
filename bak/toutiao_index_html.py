...
class ToutiaoIndexDetailSpider(Spider):
    name = 'toutiao_index_html'
    ...
    def start_requests(self):
        ...
        while not is_finish:
            ...
            for row in rows:
                user_id = row[0]
                media_id = row[1]
                url = 'https://www.toutiao.com/c/user/{}/#mid={}'.format(user_id, media_id)
    ...
    def parse_index(self, response):
        try:
            ...
            crawl_data_path = os.path.abspath('data_index_html/') + time.strftime('%Y-%m-%d', datetime.datetime.now().timetuple())
            ...
            self.logger.info('crawl toutiao index html finish: url is %s, outfile is %s' % (response.url, outfile))
            ...
            self.logger.exception('crawl toutiao index html has an error {}'.format(e))
